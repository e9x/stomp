...
export default class ClientV1 extends Client {
	...
	async #fetch_cached(
		...
		let bad_cache =
			cache === 'no-cache' || data === undefined || this.#cache_expired(data);
		...
		if (!bad_cache) {
			...
			*/
			...
			if ('etag' in data && response_data.headers.get('etag') === data.etag) {
				...
				new Date(response_data.headers.get('last-modified')).getTime() ===
		...
		if (response_data.status === 304 && cache_valid) {
			response_data = this.#read_bare_response(
				await this.#write_bare_response(
					data.body,
					data.status,
					data.statusText,
					data.headers
